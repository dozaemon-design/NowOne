...
$druft_ttl = $artwork['artwork_druft_title'] ?? ''; // 下書きタイトル artworkのみ使用
$druft_img = $artwork['artwork_druft_img'] ?? ''; // 下書き画像 artworkのみ使用
...
<article class="c-single-creation p-creation-single">
	...
	<article class="l-content l-content--inline">
			...
			<?php endif; ?>
            <?php if ($druft_ttl || $druft_img): //下書きタイトル、下書き画像 ?>
                <?php if ($druft_ttl): ?>
                    <h2 class="c-media-badge"><?php echo esc_html($druft_ttl); ?></h2>
                <?php endif; ?>

                <?php if ($druft_img): ?>
                    <figure class="p-creation-single--draft__img">
                    <?= render_creation_picture($druft_img, 'single', true); ?>
                    </figure>
                <?php endif; ?>
            <?php endif; ?>
    </article>
